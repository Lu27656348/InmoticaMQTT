...
int verify_publisher(string topic){
    redisContext* redis = redisConnect("127.0.0.1", 6379); // Conectarse a Redis
    if (redis == NULL || redis->err) {
        std::cout << "Error al conectarse a Redis: " << redis->errstr << std::endl;
        return -1;
    ...
    do {
        ...
        redisReply* reply = (redisReply*)redisCommand(redis,"HMGET publisher:%s Id", str_i.c_str());
        if (reply == nullptr) {
            std::cout << "Error al consultar Redis: " << redis->errstr << std::endl;
            redisFree(redis);
            return -1;
...
bool verify_sub_service(string topic){
    redisContext* redis = redisConnect("127.0.0.1", 6379); // Conectarse a Redis
    if (redis == NULL || redis->err) {
        std::cout << "Error al conectarse a Redis: " << redis->errstr << std::endl;
        return false;
...
void services_for_subscriber(int id)
{
    ...
    // Verificar hasta donde hay datos en la tabla "publisher"
    redisReply* ver = nullptr;
    std::string str_i = to_string(verify_sub(to_string(id)));
...
void sub_subscribe(int id)
{
    ...
    if (verify_sub(str_id) == -1) {
        cout << "No existe el siguiente suscriptor\n";
        return;
    }

    // Ingresar a qué servicio se quiere suscribir
    cout << "¿A qué servicio se quiere suscribir?\n";
    string serv;
    cin >> serv;

    // Verificar si el servicio existe
    if (!verify_sub_service(serv)) {
        cout << "No existe el servicio\n";
        return;
    }
    ...
    string str_i = to_string(verify_sub(str_id));
    ...
    if (reply->type == REDIS_REPLY_NIL) {
        ...
        string current_services = reply->str;
        if (current_services.find(serv) != string::npos) {
...
void subs_for_services(string service) {
    redisContext* redis = redisConnect("127.0.0.1", 6379); // Conectarse a Redis
    if (redis == NULL || redis->err) {
        std::cout << "Error al conectarse a Redis: " << redis->errstr << std::endl;
        return;
    ...
    cout << "Lista de Suscriptores suscritos a " << service << endl;
    do {
        str_i = to_string(i);
        ...
        if (reply->type == REDIS_REPLY_ARRAY && reply->elements >= 2) {
            if (reply->element[1]->str != nullptr){
                string current_services = reply->element[1]->str;
                if (current_services.find(service) != string::npos) {
...
int main(){
    ...
    //services_for_subscriber(248197);

    //int count = count_serv();
    //out << "numero: " << count << endl;
    ...
    //sub_subscribe(248197);

    subs_for_services("Bombillo");
